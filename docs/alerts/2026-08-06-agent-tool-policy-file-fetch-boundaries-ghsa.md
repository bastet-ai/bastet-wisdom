...
For mcp-google-search, ProjectHub-Mcp webhooks, and DataWorks MCP resources, treat every caller-controlled URL or URI as a connector capability even when the MCP server itself is local. The later mirrors place authority in a Webhooks API `url` field and `ReadResourceRequestSchema`'s `request.params.uri`; do not assume a resource URI is passive metadata. Use owned no-content peers and a denied connector recorder. Compare literal addresses, mixed/encoded address forms, DNS answers, redirects, DNS changes between policy and connect, alternate ports, user-info, and scheme transitions. Preserve the exact MCP/HTTP schema, framework coercion, resource-scheme dispatch, and parser result before URL policy runs.

Trace webhook creation through initial verification, retained configuration, later delivery, retry, and redirect handling. Trace resource reads from URI parsing through scheme/handler selection, DNS, redirects, and response serialization. The positive is **caller URL/URI passes initial validation or selects a permissive handler -> redirect or resolution selects the owned denied destination class -> connector recorder receives the final authority**. Record whether bytes would be returned to the caller, stored, or sent only as a webhook; destination control and response disclosure are separate claims. Never contact metadata, loopback applications, private production ranges, or arbitrary third parties. Repeat mcp-google-search against its committed correction and require policy to run on every redirect and the actual connected peer; treat the two new open-issue records as source-review seeds until behavior is reproduced.
