...
### Apply final-peer enforcement to MCP webpage readers

For mcp-google-search, treat `read_webpage(url)` as a connector capability even when the MCP server is local. Use owned no-content peers and a denied connector recorder. Compare literal addresses, mixed/encoded address forms, DNS answers, redirects, DNS changes between policy and connect, alternate ports, user-info, and scheme transitions. Preserve the exact MCP schema and framework coercion before URL parsing.

The positive is **caller URL passes initial validation -> redirect or resolution selects the owned denied destination class -> connector recorder receives the final authority**. Never contact metadata, loopback applications, private production ranges, or arbitrary third parties. Repeat against the committed correction and require policy to run on every redirect and the actual connected peer.
