...
- **Prefect `git_clone` branch argument injection follow-up** — [GHSA-42fh-94w4-956q](https://github.com/advisories/GHSA-42fh-94w4-956q) / CVE-2026-72538: the later record identifies a distinct pull-step `branch` path through 3.8.2, so earlier field-specific validation must not be treated as complete coverage.
...
- **Prefect branch follow-up:** patch every subprocess boundary and record argv for `commit_sha`, `directories`, branch, tag, and reference fields across clone, pull, retry, and deployment refresh. Use an owned repository and an inert option-shaped marker; deny process creation. The positive is a controlled value becoming a separate Git option, not execution.
