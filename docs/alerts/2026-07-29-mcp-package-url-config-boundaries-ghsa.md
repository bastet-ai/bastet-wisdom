...
## July 30 MCP Ruby transport follow-up

Two Ruby SDK advisories add complementary HTTP-transport checks:

- [GHSA-5p9g-j988-pcwv / CVE-2026-67431](https://github.com/advisories/GHSA-5p9g-j988-pcwv): Streamable HTTP and SSE accepted a known session ID without proving that the request came from the principal that created the session; and
- [GHSA-rjr6-rcgv-9m7m / CVE-2026-63118](https://github.com/advisories/GHSA-rjr6-rcgv-9m7m): `StreamableHTTPTransport` did not enforce `Host` or `Origin`, leaving a loopback/LAN server exposed to browser-origin and DNS-rebinding trust confusion.

The affected `mcp` gem range is through `0.22.0`; `0.23.0` is the fixed negative control. Session-ID acquisition is a separate precondition. Do not imply that the ID is guessable or disclosed merely because ownership is not checked.

### Two-principal session-ownership fixture

1. Run the exact Ruby SDK build with one inert recorder tool. The tool must accept a random marker and record principal, transport, session ID fingerprint, and arguments without performing any external action.
2. Create principals A and B through the real application authentication middleware. Let A initialize a session and obtain its synthetic session ID through normal lab instrumentation.
3. Keep the ID constant while varying the request principal: A, B, anonymous, expired A, and A after reauthentication. Test Streamable HTTP and SSE independently.
4. Send one recorder-only tool call per row. Capture the HTTP request identity, SDK-selected session owner, dispatch decision, and which stream receives the response.
5. Repeat on `0.23.0`. B and anonymous requests carrying A's ID must be rejected before dispatch; A's valid request should remain the positive control.

Strong evidence is **principal B request + A's known lab session ID -> recorder dispatch occurs under A's session**. A response arriving on A's SSE stream helps establish routing behavior but is not required to run a dangerous tool. Never obtain IDs from production logs, browser storage, traffic, or another user's session.

### Browser-origin and Host decision matrix

Use a browser harness, loopback-only Ruby MCP server, fake origin, and an inert tool. Keep DNS and all listeners inside an isolated lab; do not rebinding-test a workstation that has real filesystem, shell, cloud, or credential MCP tools loaded.

| Request row | `Host` | `Origin` | Expected result |
| --- | --- | --- | --- |
| configured local client | approved authority | absent or explicitly approved | accepted |
| foreign browser origin | approved authority | unapproved owned origin | rejected before session creation |
| Host confusion | unapproved owned authority | approved origin | rejected |
| rebinding simulation | owned name whose lab DNS answer changes to loopback | matching foreign origin | rejected |
| malformed/multiple | duplicate, absolute-form, or forwarded authority canaries | null-like or duplicate origin canaries | rejected or canonicalized consistently |

Record browser preflight behavior separately from the actual JSON-RPC request. CORS headers alone are not the DNS-rebinding control: after rebinding, browser same-origin logic may accept the attacker's name while the socket reaches loopback. A complete positive requires **foreign browser-controlled authority/origin -> local MCP transport accepts request -> inert recorder tool dispatches**. Stop there; do not list real tools or read their output.

Report session ownership and browser-origin reachability as separate boundaries. Include gem version, Rack middleware, bind address, proxy chain, transport, effective principal, session fingerprint, raw `Host`/`Origin`, DNS answer, final peer, recorder count, and the `0.23.0` negative-control result.
